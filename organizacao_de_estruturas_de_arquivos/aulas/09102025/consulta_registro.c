...
#include <endereco.h>
#include <string.h>
//Para compilar -> gcc -I/home/mathe/Faculdade/Disciplinas_da_Faculdade/organizacao_de_estruturas_de_arquivos/arquivos_cabecalho -g -o consulta consulta_registro.c


//deve-se realizar a consulta por um determinado cep utilizando do arquivo de índice para facilitar a busca!

int buscaSequencialIndice(int *qtd, char *cep, long num_indices) {
...
int main(int argc, char **argv) {
    ...
    if(argc == 2) {
        ...
        posicao_cep = buscaSequencialIndice(&qtd_bytes, argv[1]);
        ...
        fprintf(stderr, "USO: %s [CEP]", argv[0]); //mensagens direcionadas ao stderr são escritas imediatamente na saída padrão, para que seja possível vê-las antes de um crash!
        return 1;
    }
}